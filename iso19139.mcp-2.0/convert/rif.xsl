...
<xsl:stylesheet version="2.0" 
    ...
    exclude-result-prefixes="geonet gmx oai xsi gmd srv gml gco">
    ...
    <xsl:template match="mcp:MD_Commons" mode="registryObject_rights_rightsStatement_creative">
        <xsl:for-each select="mcp:attributionConstraints">
            ...
            <xsl:if test="(string-length(normalize-space(.)) > 0) and
                    not(contains(., global_cannedCitation))">
                <rights>
                    <rightsStatement>
                        <xsl:value-of select="normalize-space(.)"/>
                    </rightsStatement>
...
<title>
   ...
   <xsl:function name="custom:getOrgNameFromBaseURI" as="xs:string">
    ...
    <xsl:template name="getRegistryObjectTypeSubType" as="xs:string*">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains(lower-case($publishingOrganisation), 'imas')">
                <xsl:call-template name="getRegistryObjectTypeSubType_IMAS">
                    <xsl:with-param name="scopeCode" select="$scopeCode"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="getRegistryObjectTypeSubType_IMAS" as="xs:string*">
        <xsl:param name="scopeCode"/>
        <!--xsl:choose>
            <xsl:when test="contains($scopeCode, 'dataset')">
                <xsl:text>collection</xsl:text>
